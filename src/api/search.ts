...
): Promise<SearchResponse> => { // 使用 Promise 包裹返回类型，表示异步操作
    try {
        ...
        if (response.data.code !== 200) {
            // 如果不是成功代码，手动抛出一个错误
            // 这个错误会被下面的 catch 块捕获
            const errorMessage = response.data.message || `Backend returned non-200 code: ${response.data.code}`;
        ...
        try {
            ...
            settledResults.forEach((result, index) => {
                ...
                if (result.status === 'fulfilled') {
                    ...
                    if (key === 'byDescription') results.byDescription = result.value as SearchResponse;
                    else if (key === 'byMood') results.byMood = result.value as SearchResponse;
                    ...
                    console.error(`主搜索: 关键词 ${key} 失败！原因：`, result.reason);
